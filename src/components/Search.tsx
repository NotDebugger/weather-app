...
export default function Search({
  ...
  return (
    <div className="mt-5 mx-5 flex flex-col justify-center items-center">
      <h1 className="text-4xl text-center font-bold mb-10 font-[Bricolage_Grotesque]">
        How's the sky looking today?
      </h1>
      <form
        className="flex gap-2 w-full sm:w-fit text-sm items-start flex-col sm:flex-row"
        onSubmit={(e: React.SubmitEvent<HTMLFormElement>) => e.preventDefault()}
      >
        <div className="flex w-full sm:w-fit flex-col gap-1 relative">
        ...
        <button
          className="px-3 py-2 w-full sm:w-fit bg-indigo-700 rounded-lg cursor-pointer shadow-xl/30 hover:bg-indigo-800 transition shadow-indigo-700"
          onClick={() => {
